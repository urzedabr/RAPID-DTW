...
src/gpu/%.o: ../src/gpu/%.cpp
	...
	/usr/local/cuda-10.2/bin/nvcc -O3 -maxrregcount 32 -gencode arch=compute_75,code=sm_75  -odir "src/gpu" -M -o "$(@:%.o=%.d)" "$<"
...
src/gpu/%.o: ../src/gpu/%.cu
	...
	@echo 'Invoking: NVCC Compiler'
	/usr/local/cuda-10.2/bin/nvcc -O3 -maxrregcount 32 -gencode arch=compute_75,code=sm_75  -odir "src/gpu" -M -o "$(@:%.o=%.d)" "$<"
	/usr/local/cuda-10.2/bin/nvcc -O3 -maxrregcount 32 --compile --relocatable-device-code=false -gencode arch=compute_75,code=compute_75 -gencode arch=compute_75,code=sm_75  -x cu -o  "$@" "$<"
	@echo 'Finished building: $<'
	@echo ' '
